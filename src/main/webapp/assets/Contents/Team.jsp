
<h2 class="header">Meet the &#129433; Crew</h2>

<!-- Page Content -->
<div class="container">
    <div class="row">
        <!-- Team Member 1 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/lisa.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Lisa Kletsko</h5>
                    <div class="card-text text-black-50">Projektleitung</div>
                </div>
            </div>
        </div>
        <!-- Team Member 2 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="https://avatars0.githubusercontent.com/u/50352812?s=400&u=47cf35be185489332dfb97d818039be7a8241c51&v=4" class="card-img-top">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Patrick M&uuml;ller</h5>
                    <div class="card-text text-black-50">Backend Lead</div>
                </div>
            </div>
        </div>
        <!-- Team Member 3 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="https://media-exp1.licdn.com/dms/image/C4D03AQG26fNQfZ6VhQ/profile-displayphoto-shrink_200_200/0?e=1598486400&v=beta&t=LuFqHAgdF2ZnkKRQXFT7aKfUVea56Fj1AD4TZ0y5s3U" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Kristin Agne</h5>
                    <div class="card-text text-black-50">Frontend Lead</div>
                </div>
            </div>
        </div>
        <!-- Team Member 4 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/paul.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Paul Wolf</h5>
                    <div class="card-text text-black-50">Testmanager</div>
                </div>
            </div>
        </div>
        <!-- Team Member 5 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/henning.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Henning Sextro</h5>
                    <div class="card-text text-black-50">Backend</div>
                </div>
            </div>
        </div>
        <!-- Team Member 6 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/dave.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">David Huh</h5>
                    <div class="card-text text-black-50">Backend</div>
                </div>
            </div>
        </div>
        <!-- Team Member 7 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/danny.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Danny Kroll</h5>
                    <div class="card-text text-black-50">Frontend</div>
                </div>
            </div>
        </div>
        <!-- Team Member 8 -->
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="card border-0 shadow">
                <img src="../../assets/Images/Team/nico.png" class="card-img-top" alt="...">
                <div class="card-body text-center">
                    <h5 class="card-title mb-0">Nico Holzh&auml;user</h5>
                    <div class="card-text text-black-50">&#129433;&#129433;</div>
                </div>
            </div>
        </div>
    </div>
    <!-- /.row -->
</div>
<!-- /.container -->
<c:if test="${!sessionBean.loggedIn}">
    <div class="center">
        <hr>
    </div>
    <div class="text-center">
        <a href="./">Zur&uuml;ck zur Login-Seite</a>
    </div>
</c:if>